...
for line in lines:
    if line.startswith("                if (selectedTabIndex == 0) {"):
        in_block = True
        brace_count = 1
        # Insert our new code here
        out_lines.append("""                if (selectedTabIndex == 0) {
...
""")
    ...
    if in_block:
        ...
        brace_count -= line.count('}')
        if brace_count == 0 and line.strip() == "} else {":
            in_block = False
...
with open('app/src/main/java/yangfentuozi/dsusideloaderplus/ui/screen/gsihub/GsiHubScreen_fixed.kt', 'w') as f:
