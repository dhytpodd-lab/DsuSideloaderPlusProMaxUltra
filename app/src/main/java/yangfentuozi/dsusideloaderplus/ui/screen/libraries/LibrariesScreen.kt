...
fun LibrariesScreen(
    ...
    ApplicationScreen(
        ...
        LazySplicedColumnGroup(
            ...
            val titleName = when (library.name) {
                "hidden-api-bypass" -> "Hidden API Bypass"
                "hidden-api-stub" -> "Hidden API Stub"
                "shizuku-api" -> "Shizuku API"
                "shizuku-provider" -> "Shizuku Provider"
                "capsule" -> "Capsule"
                "libsu" -> "libsu"
                "libsu-core" -> "libsu Core"
                "libsu-service" -> "libsu Service"
                else -> library.name
            }
            SettingsItem(
                title = titleName,
